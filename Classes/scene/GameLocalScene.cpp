...
bool Game::init()
{
	...
	if (ptr != nullptr)
	{
		...
		_sceneManager = Manager::create("Steven", *this);
		this->addChild(_sceneManager);
	}
	...
	for (int i = 1; i < 34; i++)
	{
		for (int j = 1; j < 24; j++)
		{
			if (isTouchDiamond(Vec2(16 + i * 32, 16 + j * 32)))
			{
				...
				_bonusNumber++;
				this->addChild(bonus, 1, _bonusNumber);
			}
		}
	}
	for (int i = 1; i < 34; i++)
	{
		for (int j = 1; j < 24; j++)
		{
			if (isTouchFinalLadder(Vec2(16 + i * 32, 16 + j * 32)))
			{
				...
				this->addChild(ladder, 2, 20 + _ladderNumber);
			}
		}
	}
}
...
//==============重力检测========================================
bool Game::bGravityAir(Vec2 position)
{
...
*/
	...
	{
		pos[i] = _remotePlayer[_playerList.at(i).player_name]->getPosition();
	}
	if (pos[0] == pos[1])
	{
		for (int i = 0; i < _playerList.size(); i++)
		{
			...
			_client->sendMessage(code, _remotePlayer[_playerList.at(i).player_name]->getPlayerName());
			_remotePlayer[_playerList.at(i).player_name]->setPlayerDead();

		}
	}
}

void Game::LadderVisible()
{
	if (_rank == _bonusNumber - 1)
	{
		for (int i = 1; i <= _ladderNumber; ++i)
		{
